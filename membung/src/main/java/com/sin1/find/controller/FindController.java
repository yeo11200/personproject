package com.sin1.find.controller;
...
import com.sin1.find.service.ReplyFindService;
import com.sin1.find.service.FindService;
...
public class FindController {
	...
	public String list(Model model, PageObject pageObject) {
		...
		model.addAttribute("list", findService.list(pageObject));
	...
	public String view(Model model, int no, @RequestParam(value="isView", defaultValue="false")boolean isView, ReplyPageObject pageObject) {
		// pageObject
		model.addAttribute("view", findService.view(no,isView));
		// 댓글에 관한 리스트 pageObject -> ReplyBoardService.
		model.addAttribute("ReplyList", replyFindService.list(pageObject));
		// pageObject
		// model.addAtttribute("page",pageObject);
		model.addAttribute("page", pageObject);
		return "sell/view";
	...
	public String write(FindDTO dto, HttpSession session, RedirectAttributes rs) throws IOException {
		// 넘어온 파일을 파일로 저장을 한다.
...
//		System.out.println(fileSize);
				String realPath = session.getServletContext().getRealPath(findPath);
				System.out.println(dto);
	...
	public String updateForm(int no, Model model) {
		model.addAttribute("update", findService.view(no, false));
		return "sell/update";
	}
	...
	public String update(FindDTO dto, HttpSession session, RedirectAttributes rs) throws Exception {
		...
		String realPath = session.getServletContext().getRealPath(findPath);
		findService.update(realPath, dto);
	...
	public String delete(int no, HttpSession session, RedirectAttributes rs) {
		...
		String realPath 
		= session.getServletContext().getRealPath(findPath);
		findService.delete(realPath, no);
		return "redirect:/sell/list.do";
	}
...
//	}
	...
	public String ReplyWrite(ReplyFindDTO dto) {
		System.out.println(dto);
		replyFindService.ReplyWrite(dto);
	...
	public String ReplyUpdateForm(Model model, int rno) {
		model.addAttribute("ReplyUpdate", replyFindService.view(rno));
		return "sell/ReplyUpdate";
	}
	
	// 댓글 수정
	@RequestMapping(path="ReplyUpdate.do", method=RequestMethod.POST)
	public String ReplyUpdate(ReplyFindDTO dto) {
		replyFindService.ReplyUpdate(dto);
		return "redirect:/sell/view.do?no="+dto.getNo()+"&isView=false";
	}
	...
	public String ReplyDelete(int rno, int no) {
		replyFindService.ReplyDelete(rno);
