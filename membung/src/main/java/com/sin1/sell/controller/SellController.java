package com.sin1.sell.controller;
...
import com.sin1.sell.service.ReplySellService;
...
public class SellController {
	...
	@Autowired
	private ReplySellService replySellService;
	...
	public String list(Model model, PageObject pageObject) {
		...
		model.addAttribute("list", sellService.list(pageObject));
	...
	public String view(Model model, int no, @RequestParam(value="isView", defaultValue="false")boolean isView, ReplyPageObject pageObject) {
		// pageObject
		model.addAttribute("view", sellService.view(no,isView));
		// 댓글에 관한 리스트 pageObject -> ReplyBoardService.
		model.addAttribute("ReplyList", replySellService.list(pageObject));
		// pageObject
		// model.addAtttribute("page",pageObject);
		model.addAttribute("page", pageObject);
		return "sell/view";
	...
	public String write(SellDTO dto, HttpSession session, RedirectAttributes rs) throws IOException {
		// 넘어온 파일을 파일로 저장을 한다.
...
//		System.out.println(fileSize);
	...
	public String updateForm(int no, Model model) {
		model.addAttribute("update", sellService.view(no, false));
		return "sell/update";
	}
	...
	public String update(SellDTO dto, HttpSession session, RedirectAttributes rs) throws Exception {
		...
		String realPath = session.getServletContext().getRealPath(imagePath);
		sellService.update(realPath, dto);
	...
	public String delete(int no, HttpSession session, RedirectAttributes rs) {
		...
		String realPath 
		= session.getServletContext().getRealPath(imagePath);
		sellService.delete(realPath, no);
		return "redirect:/sell/list.do";
	}
...
//	}
	...
	public String ReplyWrite(ReplySellDTO dto) {
		System.out.println(dto);
		replySellService.ReplyWrite(dto);
	...
	public String ReplyUpdateForm(Model model, int rno) {
		model.addAttribute("ReplyUpdate", replySellService.view(rno));
		return "sell/ReplyUpdate";
	}
	
	// 댓글 수정
	@RequestMapping(path="ReplyUpdate.do", method=RequestMethod.POST)
	public String ReplyUpdate(ReplySellDTO dto) {
		replySellService.ReplyUpdate(dto);
		return "redirect:/sell/view.do?no="+dto.getNo()+"&isView=false";
	}
	...
	public String ReplyDelete(int rno, int no) {
		replySellService.ReplyDelete(rno);
